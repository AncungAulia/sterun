...
| --- | --- | --- | --- |
| `/` | Poster-first directory (redesigned 2026-09-11, spec `docs/superpowers/specs/2026-09-11-directory-redesign-design.md`, Revision 2): one list of races, filtered by the place chosen at the top left ("All locations" by default, or a country with an optional province, saved in the visitor's browser), which narrows the featured row of `Open`, upcoming races that have a poster too. The list is headed "All races", "Races in {place}" or "{n} races match"; a place with no races offers "See all locations". Every race is a card with its poster in a 16:9 frame (shown whole, never cropped; "No image" without one), venue, date, entries left and starting price, up to 4 columns wide. Search, plus a filter drawer: sort by date, price, distance, hide full and closed races. Loading + empty + error states. | the chain (RPC) for the races, plus each event's verified metadata document for poster and location | STE-13 |
