...
| --- | --- | --- | --- |
| `/` | Poster-first directory (redesigned 2026-09-11, spec `docs/superpowers/specs/2026-09-11-directory-redesign-design.md`, Revision 3): one list of every race, sorted by the place chosen at the top left ("All locations" by default, or a country with an optional province, saved in the visitor's browser). The place sorts, it does not filter: races in it come first, within "still to come" and within "already run" rather than across them, and the featured row of `Open`, upcoming races with a poster prefers them, but nothing is hidden. The list is headed "All races", or "{n} races match" once a search or filter narrows it, with a line under the heading naming the place while one is chosen and nothing is narrowing the list. Every race is a card with its poster in a 16:9 frame (shown whole, never cropped; "No image" without one), venue, date, entries left and starting price, up to 4 columns wide. Search, plus a filter drawer: sort by date, price, distance, hide full and closed races. Loading + empty + error states. | the chain (RPC) for the races, plus each event's verified metadata document for poster and location | STE-13 |
