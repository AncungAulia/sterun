...
format is defined **twice on purpose**, here and in `be/src/announcements.ts`, for the same reason as
the error tables: `be/` does not depend on this package at runtime. Both are pinned to
`schema/announcement-v1.vectors.json`, which was generated by a third, independent implementation, so
neither copy is checked only against itself.

Two traps, both met while building it:

- **`hash` from `@stellar/stellar-sdk` is typed as a `Uint8Array`.** `.toString("hex")` on it ignores the
  argument and returns `"12,34,..."`, so wrap it in `Buffer.from(...)`. The vectors caught it: the message
  was wrong, and no signature would ever have verified.
- **`verifyAnnouncement` proves who signed, not who may speak.** A valid signature from a stranger means
  nothing; compare the signer with `getEvent(eventId).organiser`.

Never change the v1 format; a new format is a new header line and a new vectors file.
