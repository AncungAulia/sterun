...
/** What the browser answered, which names no country at all. */
const NEAR: Place = { mode: "nearby", lat: -7.7828, lng: 110.3671 };
...
describe("AreaPicker", () => {
  describe("positive", () => {
    it("reads All locations when no place is chosen", () => {
      render(<AreaPicker place={null} onSave={vi.fn()} onClear={vi.fn()} />);
    ...
    it("reads Near you while the browser's own answer is in use", () => {
      // Not a province name: turning two numbers into one needs a geocoding
      // service, and this project uses none.
      render(<AreaPicker place={NEAR} onSave={vi.fn()} onClear={vi.fn()} />);

      expect(screen.getByRole("button", { name: "Near you" })).toBeInTheDocument();
    });

    it("still offers the lists over coordinates, and a province replaces them", async () => {
      const onSave = vi.fn();
      render(<Harness initial={NEAR} onSave={onSave} />);

      await openPicker("Near you");
      await userEvent.click(await screen.findByRole("combobox", { name: "Province" }));
      await userEvent.click(await screen.findByRole("option", { name: "DI Yogyakarta" }));
      await userEvent.click(screen.getByRole("button", { name: "Apply" }));
    ...
    it("offers a way back out of Near you", async () => {
      // Coordinates need the clear option as much as a province does: nothing
      // else on the page turns them off.
      const onClear = vi.fn();
      render(<Harness initial={NEAR} onClear={onClear} />);

      const dialog = await openPicker("Near you");
      await userEvent.click(within(dialog).getByRole("button", { name: "All locations" }));
  ...
  describe("edge", () => {
    ...
    it("starts on All of the saved country when no province was saved", async () => {
      render(<AreaPicker place={{ mode: "area", countryCode: "MY", country: "Malaysia" }} onSave={vi.fn()} onClear={vi.fn()} />);
    ...
    it("goes back to the whole country when another country is picked", async () => {
      ...
      await userEvent.click(screen.getByRole("button", { name: "Apply" }));
      expect(onSave.mock.calls[0][0]).toStrictEqual({ mode: "area", countryCode: "MY", country: "Malaysia" });
    });

    it("takes a typed province for a country the data has no provinces for", async () => {
      const onSave = vi.fn();
      render(<AreaPicker place={null} onSave={onSave} onClear={vi.fn()} />);
    ...
    it("saves the whole country when that typed province is left blank", async () => {
      ...
      expect(onSave.mock.calls[0][0]).toStrictEqual({ mode: "area", countryCode: "GI", country: "Gibraltar" });
    });
  });
  ...
  describe("negative", () => {
    it("offers nothing to clear when no place is saved", async () => {
      const onClear = vi.fn();
      render(<AreaPicker place={null} onSave={vi.fn()} onClear={onClear} />);
