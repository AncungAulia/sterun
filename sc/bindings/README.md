...
> **Regenerating wipes the package directory**, `package-lock.json` included, because
> `--overwrite` rewrites the whole `--output-dir`. The lockfiles are committed and are not generator
> output, so restore them (`git checkout -- sc/bindings/*/package-lock.json`) after a regeneration
